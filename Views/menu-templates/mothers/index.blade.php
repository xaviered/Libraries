    ...
    <table>
        ...
        @foreach($template->categories as $category)
            @if($category->slug === 'main_course')
                ...
                @include( '/ixavier-libraries/menu-templates/mothers/table-list-products', ['category'=>$category] )
            @else
                <tr class="category category-{{ $category->slug }} category-{{ $categoryIndex }}">
                    <td class="title" colspan="3">{{ $category->title }}</td>
                </tr>
                <tr class="products">
                    <td colspan="3" class="category-{{ $category->slug }} category-{{ $categoryIndex }}">
                        @include( '/ixavier-libraries/menu-templates/mothers/block-list-products', ['category'=>$category] )
                    </td>
                </tr>
            @endif
            <?php $categoryIndex++ ?>
